...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            #-------------------------------------
            cam_w ,cam_h=1000,500
            fps=80
            cam.Width.set(cam_w)
            cam.Height.set(cam_h)
            cam.stream_on()
            ...
            img_bgr  = rgb_image.get_numpy_array()
            if img_bgr  is None:
                continue
            self.imgs[i] = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
            w = int(cam.Width.get())
            h = int(cam.Height.get())
            ...
            self.threads[i] = Thread(target=self.update, args=([i, raw_image, s]), daemon=True)
    ...
    def update(self, i, raw_image, stream):
